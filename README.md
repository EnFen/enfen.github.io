...
Navigation on the site includes links between and within pages, so that users can quickly reference necessary information.

## Sitemap
The relationship between the pages and stylesheets contained in the site can be seen in the following digram:

<img src="/docs/img/portfolio_sitemap.jpg" alt="Portfolio Sitemap" title="Sitemap" />

## Screenshots

The following are images show the final site, and some features on different devices.
...
## Target audience
I hope to attract the attention of potential clients or employers seeking web development services with the website, with the ultimate goal beinging contacted by them with a request for work. That said, I'd love to include some 'fun stuff' in future iterations of the site which would make my profile page an enjoyable and memorable visit for anyone :}

## Tech stack
As at the completion of this project, the site has been built using HTML5 for structure and SCSS/CSS3 for styling. Upon completion it was deployed through Git Pages.

# Design
For site aesthetics I wanted a slight 'Tech' feel. I felt this was an appropriate for a profile page meant to showcase technical skill, but also spoke to my love of gaming and sci-fi / fantasy.
...
To best showcase the information on the site, I wanted the layout to be minimalist and clean, to fit with the modern 'Tech' aesthetic, and to allow lots of visual space for page elements. This would also assist in directing users to the information on the site that I felt was most imortant (About, Contact). Consequently, I felt that seperating information onto different pages, and having diffiernt layouts for each of the mobile, tablet, and desktop sizes was the best way to achieve this.
...
The final asset I required was my photo. I wanted a candid shot which showed my personality, and which worked well with the sites colour scheme. I also put some thought into how the photo would aid the purpose of the site - so I chose to  pose for the photo looking towards the top-right, so that my eyeline would direct a users gaze, first across the featured content in the center of the page, and then towards the navigation menus in the top-right.
...
A request can be issued by the user of a browser by submitting the IP address of the desired files, however for humans this is most often achieved using a domain name which is far easier to remember than the millions of possible IP address numbers. This domain leads to the information physically stored on a web server.

Web servers can refer to hardware or software. As hardware they represent the location where internet content is stored (or hosted), and  may include other software to run and optimise the information they host. As software, web servers refer to applications that ‘listen’ for requests from clients (such as browsers) and then ‘serve’ the requested information via packets back to the requestor, according to predefined protocols (most often HTTP for the web).
...
Internationalisation of websites describes the process of translating web content into other languages so that information is accessible to a greater variety of cultures
...
Thanks to the efforts of these technologies and agencies, anyone, regardless of any particular constraint, has more opportunity to be connected to information than ever before – just like everyone else - and this makes business, society, and all peoples lives better.
